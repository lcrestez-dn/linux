...
static void tcp_event_new_data_sent(struct sock *sk, struct sk_buff *skb)
{
	...
	if (interesting_sk(sk)) {
		QP_PRINT_LOC("sk=%p advance snd_nxt=%u to %u from %ps\n",
				sk, tp->snd_nxt, TCP_SKB_CB(skb)->end_seq, __builtin_return_address(0));
	}
...
/*
 * Wait for enough data to sent an mtu probe or send non-probes?
 *
 * RFC4821 7.4: "Protocols MAY delay sending non-probes in order to accumulate
 * enough data to meet the pre-conditions for probing"
 */
bool tcp_mtu_probe_should_wait(struct sock *sk)
{
	...
	return net->ipv4.sysctl_tcp_probe_wait && icsk->icsk_mtup.wait_data;
}
...
static int tcp_mtu_probe(struct sock *sk)
{
	...
	if (tp->snd_cwnd < net->ipv4.sysctl_tcp_probe_cwnd_threshold) {
		if (interesting_sk(sk)) {
			QP_PRINT_LOC("sk=%p skip snd_cwnd=%d\n", sk, tp->snd_cwnd);
		}
		return -1;
			   ...
			   int push_one, gfp_t gfp)
{
	 ...
	 * If user does many small writes 
	 */
	if (!push_one || (push_one < 2 && net->ipv4.sysctl_tcp_probe_wait)) {
		...
		if (!result) {
			if (interesting_sk(sk)) {
				QP_PRINT_LOC("sk=%p probe wait push_one=%d\n", sk, push_one);
			}
			...
			icsk->icsk_mtup.wait_data = false;
			if (interesting_sk(sk)) {
				QP_PRINT_LOC("sk=%p mtu probe not applicable\n", sk);
			}
		}
	}
